...
The following is an example that $X^tX$ is well-conditioned, while the Hessian matrix is not due to a extreme vale of $\beta$, namely $p$ is close to 1.
To make this vignette work, we set 'eval = F' for the second block. Otherwise, R will throw an error.
```{r}
n<-5000
p<-3
